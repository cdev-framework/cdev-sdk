...
from cdev.resources.simple.commands.table.utils import (
    _validate_data,
    recursive_translate_data,
)
...
class put_object(BaseCommand):
    ...
    def command(self, *args, **kwargs):
        ...
        attributes_dict = {
            x.get("AttributeName"): x.get("AttributeType") for x in resource.attributes
        }
        ...
        try:
            raw_aws_client.run_client_function(
                "dynamodb",
                "put_item",
                {"TableName": dynamodb_table_name, "Item": translated_data},
            )
        except Exception as e:
