...
def _create_simple_lambda(
    identifier: str, resource: simple_lambda.simple_aws_lambda_function_model
) -> bool:
    ...
    print_deployment_step(
        "CREATE", f"Creating lambda function resources for lambda {resource.name}"
    )
    ...
    print_deployment_step(
        "CREATE", f"  Create role for lambda function {resource.name}"
    )
    ...
    print_deployment_step(
        "CREATE", f"  Upload code for lambda function {resource.name}"
    )
    ...
    if resource.external_dependencies_info:
        print_deployment_step(
            "CREATE", f"  Creating dependencies for lambda function {resource.name}"
        )
    ...
    # TODO
    print_deployment_step(
        "CREATE", f"  [blink]Waiting for role to finish creating (~10s)[/blink]"
    )
    sleep(10)
    ...
    lambda_function_args = {
        ...
        "Environment": resource.configuration.Environment.dict()
    ...
    lambda_function_rv = raw_aws_client.run_client_function(
    ...
    print_deployment_step(
        "CREATE", f"  Created lambda function resources for lambda {resource.name}"
    )
    return True


def _upload_s3_code_artifact(
    resource: simple_lambda.simple_aws_lambda_function_model,
) -> str:
    ...
    log.debug(
        f"artifact keyname {keyname}; ondisk location {zip_location}; is valid file {os.path.isfile(zip_location)}"
    )
...
def _create_dependency(
    resource: simple_lambda.simple_aws_lambda_function_model, dependency: Dict
) -> Dict:
    ...
    dependency_rv = raw_aws_client.run_client_function(
        "lambda",
...
def _remove_simple_lambda(
    ...
    permissions = cdev_cloud_mapper.get_output_value_by_hash(
        resource.hash, "permissions"
    )
    ...
    print_deployment_step(
        "DELETE", f"  Remove role for lambda function {resource.name}"
    )

    log.debug(f"Attempting to delete dependencies")
    dependencies_info = cdev_cloud_mapper.get_output_value_by_hash(
        resource.hash, "layers"
    )
    if dependencies_info:
        ...
        print_deployment_step(
            "DELETE", f"  Remove dependencies for lambda function {resource.name}"
        )
...
def _update_simple_lambda(
    previous_resource: simple_lambda.simple_aws_lambda_function_model,
    new_resource: simple_lambda.simple_aws_lambda_function_model,
) -> bool:
    # Updates can be of:
    ...
    if not previous_resource.config_hash == new_resource.config_hash:
        if (
            not previous_resource.configuration.get("Environment")
            == new_resource.configuration.Environment
        ):
            log.debug(
                f"UPDATE ENVIRONMENT VARIABLES: {previous_resource.configuration.get('Environment')} -> {new_resource.configuration.Environment}"
            )
            raw_aws_client.run_client_function(
                "lambda",
                "update_function_configuration",
                {
                    "FunctionName": cdev_cloud_mapper.get_output_value_by_hash(
            ...
            print_deployment_step(
                "UPDATE",
                f"  Update environment variabels for lambda function {new_resource.name}",
            )
    ...
    if not previous_resource.permissions_hash == new_resource.permissions_hash:
        ...
        previous_hashes = set(
            [
                simple_lambda.Permission(**x).get_hash()
                if "resource" in x
                else simple_lambda.PermissionArn(**x).get_hash()
                for x in previous_resource.permissions
            ]
        )
        ...
        for permission in previous_resource.permissions:
            previous_resource_permission_hash = (
                simple_lambda.Permission(**permission).get_hash()
                if "resource" in permission
                else simple_lambda.PermissionArn(**permission).get_hash()
            )
        ...
        permission_output = cdev_cloud_mapper.get_output_value_by_hash(
            previous_resource.hash, "permissions"
        )
        role_name_output = cdev_cloud_mapper.get_output_value_by_hash(
            previous_resource.hash, "role_name"
        )
        ...
        for permission in remove_permissions:
            ...
            delete_policy(
                role_name_output,
                permission_output.get(previous_resource_permission_hash),
            )
            permission_output.pop(previous_resource_permission_hash)

        cdev_cloud_mapper.update_output_by_key(
            previous_resource.hash, "permissions", permission_output
        )
        print_deployment_step(
            "UPDATE", f"  Update permissions for lambda function {new_resource.name}"
        )

    if not previous_resource.src_code_hash == new_resource.src_code_hash:
        log.debug(
            f"UPDATE SOURCE CODE OF {previous_resource.name}; {previous_resource.src_code_hash} -> {new_resource.src_code_hash}"
        )
        ...
        print_deployment_step(
            "UPDATE", f"  Update source code for lambda function {new_resource.name}"
        )

    if (
        not previous_resource.external_dependencies_hash
        == new_resource.external_dependencies_hash
    ):
        log.debug(
            f"UPDATE DEPENDENCIES OF {previous_resource.name}; {previous_resource.external_dependencies_hash} -> {new_resource.external_dependencies_hash}"
        )
        rv = []
        dependencies_info = cdev_cloud_mapper.get_output_value_by_hash(
            previous_resource.hash, "layers"
        )
        ...
        if new_resource.external_dependencies_info:
            new_layer_rv = _create_dependency(
                new_resource, new_resource.external_dependencies_info
            )
            ...
            raw_aws_client.run_client_function(
                ...
                "update_function_configuration",
                {
                    ...
                    "Layers": [
                        f'{new_layer_rv.get("arn")}:{new_layer_rv.get("version")}'
                    ],
                },
            )
    ...
    print_deployment_step(
        "UPDATE", f"  Finished updating lambda function {new_resource.name}"
    )
    return True
