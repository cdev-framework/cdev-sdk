...
def create_role_with_permissions(
    role_name: str,
    permissions: List[Union[simple_lambda.Permission, simple_lambda.PermissionArn]],
...
def delete_role_and_permissions(
    role_name: str, permission_arns: List[Tuple[str, bool]]
) -> bool:
    """
    ...
    raw_aws_client.run_client_function("iam", "delete_role", {"RoleName": role_name})
...
def delete_policy(role_name: str, permission_arn: Tuple[str, bool]) -> bool:
    ...
    raw_aws_client.run_client_function(
        "iam",
        "detach_role_policy",
        {"RoleName": role_name, "PolicyArn": permission_arn[0]},
    )

    if permission_arn[1]:
        raw_aws_client.run_client_function(
            "iam", "delete_policy", {"PolicyArn": permission_arn[0]}
        )
...
def add_policy(
    ...
    if isinstance(permission, simple_lambda.PermissionArn):
        ...
        is_created = True
        log.debug(
            f"Created Permission {returned_permission_arn} from {permission.get_hash()}"
        )
...
def _create_policy(permission: simple_lambda.Permission) -> str:
    ...
    resource_info = cloud_mapper_manager.get_output_by_name(
        cdev_resource_type, cdev_resource_name
    )
    ...
    statement["Resource"] = (
        resource_info.get("arn")
        if not permission.resource_suffix
        else f"{resource_info.get('arn')}{permission.resource_suffix}"
    )
