...
def create_project_diffs(
    new_project_state: List[ComponentModel], previous_local_state: List[ComponentModel]
) -> Tuple[List[Component_Difference], List[Resource_Difference]]:
    """
    ...
    for component in new_project_state:
        if (
            ...
            resource_diffs = _create_resource_diffs(
                component.uuid, component.rendered_resources, []
            )
            ...
            previous_components_to_remove.remove(
                previous_name_to_component.get(component.name)
            )
            ...
            component_rv.append(
                Component_Difference(
                    **{
                        ...
                        "previous_name": previous_hash_to_component.get(
                            component.name
                        ).name,
                        "new_name": component.name,
                    }
            ...
            # POP the seen previous component as we go so only remaining resources will be deletes
            previous_components_to_remove.remove(
                previous_hash_to_component.get(component.hash)
            )
...
def generate_sorted_resources(
    resource_diffs: List[Resource_Difference],
) -> Generator[Resource_Difference, None, None]:
    ...
    new_resource_id_to_resource_diff = {
        f"{x.component_uuid};{x.new_resource.ruuid};{x.new_resource.hash}": x
        for x in resource_diffs
        if x.new_resource
    }

    # this is used as the first check in the translation from parent_references by name
    new_resource_name_to_resource_diff = {
        f"{x.component_uuid};{x.new_resource.ruuid};{x.new_resource.name}": x
        for x in resource_diffs
        if x.new_resource
    }

    # build previous dict from {x.new_resource.ruuid}::{x.new_resource.hash} to resource
    previous_resource_id_to_resource_diff = {
        f"{x.component_uuid};{x.previous_resource.ruuid};{x.previous_resource.hash}": x
        for x in resource_diffs
        if not x.new_resource
    }

    # this is used as the first check in the translation from parent_references by name
    previous_resource_name_to_resource_diff = {
        f"{x.component_uuid};{x.previous_resource.ruuid};{x.previous_resource.name}": x
        for x in resource_diffs
        if not x.new_resource
    }
    ...
    for resource_diff in resource_diffs:
        if resource_diff.new_resource:
            ...
            resource_id = (
                f"{resource_diff.new_resource.ruuid};{resource_diff.new_resource.hash}"
            )
            if resource_diff.new_resource.parent_resources:
                ...
                for parent_reference in resource_diff.new_resource.parent_resources:
                    ...
                    if parent_resource_split[1] == "name":
                        ...
                        name_identifier = (
                            f"{parent_resource_split[0]};{parent_resource_split[2]}"
                        )
                        if name_identifier in new_resource_name_to_resource_diff:
                            parent_resource = new_resource_name_to_resource_diff.get(
                                name_identifier
                            )
                        ...
                        if hash_identifier in new_resource_id_to_resource_diff:
                            parent_resource = new_resource_id_to_resource_diff.get(
                                hash_identifier
                            )
                    ...
                    if parent_resource_id in seen_parents:
                        log.debug(
                            f"Already seen parent {parent_resource_id} for {resource_diff.new_resource.name}; {seen_parents}"
                        )
                        continue

                    log.info(
                        f"PARENT RESOURCE {parent_resource_id} for {resource_diff.new_resource.name}"
                    )
            ...
            if resource_diff.previous_resource.parent_resources:
                ...
                for (
                    parent_reference
                ) in resource_diff.previous_resource.parent_resources:
                    # A parent reference can be either ruuid:hash:<hash> or ruuid:name:<name>
                    # if the parent is reference by name means we need to look up the current hash of that parent.
                    parent_resource_split = parent_reference.split(";")
                    ...
                    if parent_resource_split[1] == "name":
                        ...
                        if name_identifier in previous_resource_name_to_resource_diff:
                            parent_resource = (
                                previous_resource_name_to_resource_diff.get(
                                    name_identifier
                                )
                            )
                        ...
                        if hash_identifier in previous_resource_id_to_resource_diff:
                            parent_resource = previous_resource_id_to_resource_diff.get(
                                hash_identifier
                            )
                    ...
                    if parent_resource_id in seen_parents:
                        log.debug(
                            f"Already seen parent {parent_resource_id} for {resource_diff.previous_resource.name}; {seen_parents}"
                        )
                        continue

                    log.info(
                        f"PARENT RESOURCE {parent_resource_id} for {resource_diff.previous_resource.name}"
                    )
                    # IF the parent is in the diff make this resource a descandant in the DAG
                    resource_dag.add_node(parent_resource_id)
