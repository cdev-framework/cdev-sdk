...
def find_specified_command(command_list: List[str], all_search_locations_list: List[str]) -> Tuple[Union[BaseCommand,BaseCommandContainer], str, str, bool]:
...
def find_unspecified_command(command: str, all_search_locations_list: List[str]) -> Tuple[Union[BaseCommand,BaseCommandContainer], str, str, bool]:
...
def initialize_command_module(mod_path: str) -> BaseCommand:
    ...
    for item in dir(mod):  
        ...
        if inspect.isclass(potential_obj) and issubclass(potential_obj, BaseCommand) and not (potential_obj == BaseCommand):
...
def initialize_command_container_module(mod_path: str) -> BaseCommandContainer:
    ...
    for item in dir(mod):
        potential_obj = getattr(mod,item)  
        if inspect.isclass(potential_obj) and issubclass(potential_obj, BaseCommandContainer) and not (potential_obj == BaseCommandContainer):
