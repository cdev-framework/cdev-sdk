...
def get_boto_client(service_name):
    ...
    if (
        cdev_settings.SETTINGS.get("CREDENTIALS").get("credentials_type")
        == "raw_credentials"
    ):
        return _get_boto_client(
            service_name,
            credentials=cdev_settings.SETTINGS.get("CREDENTIALS").get("value"),
        )

    if cdev_settings.SETTINGS.get("CREDENTIALS").get("credentials_type") == "profile":
        return _get_boto_client(
            service_name,
            profile_name=cdev_settings.SETTINGS.get("CREDENTIALS").get("value"),
        )
