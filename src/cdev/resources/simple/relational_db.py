...
class RelationalDB(Cdev_Resource):
    ...
    def __init__(self, cdev_name: str, engine: db_engine, username: str, password: str, httpendpoint: bool=True, cluster_name: str="", 
        ...
        self.DBClusterIdentifier = f"{cluster_name}-{cdev_environment.get_current_environment_hash()}" if cluster_name else f"{cdev_name}-{cdev_environment.get_current_environment_hash()}"
