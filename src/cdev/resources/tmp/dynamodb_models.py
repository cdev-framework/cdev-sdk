...
*  `StreamViewType` - When an item in the table is modified, `StreamViewType` determines what information is written to the table's stream. Valid values for `StreamViewType` are:
 ...
 For an overview on tagging DynamoDB resources, see [Tagging for DynamoDB](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Tagging.html) in the *Amazon DynamoDB Developer Guide*.
    ...
    def filter_out_extra(self) -> dict:
        NEEDED_ATTRIBUTES = set([self.AttributeDefinitions, self.TableName, self.KeySchema, self.LocalSecondaryIndexes, self.GlobalSecondaryIndexes, self.BillingMode, self.ProvisionedThroughput, self.StreamSpecification, self.SSESpecification, self.Tags])
