...
def _create_simple_queue(identifier: str, resource: simple_queue.simple_queue_model) -> bool:
    attributes = {}

    if resource.fifo:
        attributes.update({
            "FifoQueue": str(resource.fifo)
        })
    ...
    rv = raw_aws_client.run_client_function("sqs", "get_queue_attributes", {
        "QueueUrl": output_info.get("queue_url"),
...
def handle_simple_queue_deployment(resource_diff: Resource_State_Difference) -> bool:
    try:
        if resource_diff.action_type == Action_Type.CREATE:
            return _create_simple_queue(resource_diff.new_resource.hash, resource_diff.new_resource)
        elif resource_diff.action_type == Action_Type.UPDATE_IDENTITY:
            return _update_simple_queue(resource_diff.previous_resource, resource_diff.new_resource)
        elif resource_diff.action_type == Action_Type.DELETE:
            return _remove_simple_queue(resource_diff.previous_resource.hash, resource_diff.previous_resource)
