...
class weighted_dependency_graph:
    ...
    def get_total_weight(self) -> int:
        ...
        for node in self._top_level_nodes:
            subtree_unique_weight, _already_seen_module = self._recursive_compute_weight(node, _already_seen_module)
    ...
    def _recursive_compute_weight(self, node: weighted_dependency_node, already_seen: Set[weighted_dependency_node]) -> Tuple[int, Set[weighted_dependency_node]]:
        weight = 0 
        ...
        if node.get_children():
            for child in node.get_children():
                subtree_unique_weight, already_seen_module = self._recursive_compute_weight(child, already_seen_module)
...
def compute_index(dependency_graph: weighted_dependency_graph, number_of_top_level_modules_to_remove: int=2) -> Tuple[Dict, List[str]]:
    ...
    _start_removals = list(combinations(_top_level_ids, number_of_top_level_modules_to_remove)) 
    ...
    for start_removal in _start_removals:
        ...
        for top_leaf_child in dependency_graph.HEAD.get_children():
            if top_leaf_child.id in t:
                t2 = t.union(set(["HEAD"]))
                weight_pruned, already_removed = _recursively_compute_removal(top_leaf_child, t2, already_removed)
                weight_removed += weight_pruned
            else:
                weight_pruned, already_removed = _recursively_compute_removal(top_leaf_child, t, already_removed)
                weight_removed += weight_pruned
    ...
    if len(remaining_parents) == 0:
        ...
        if current_node.get_children():
            for child in current_node.get_children():
                weight_pruned, already_removed  = _recursively_compute_removal(child, modules_removing, already_removed)
