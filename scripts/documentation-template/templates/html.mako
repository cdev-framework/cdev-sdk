<%
  ...
  def link(dobj: pdoc.Doc, name=None, sidebar=False):
    ...
    classes =  "sidebar" if sidebar else ""

    return '<a class="{}" title="{}" href="{}">{}</a>'.format(classes, dobj.refname, url, name)
...
<%def name="module_index(module)">
  ...
  <nav id="sidebar">
    ...
    <li><h3>Super-module</h3>
      <ul>
        <li><code>${link(supermodule, sidebar=True)}</code></li>
      </ul>
    </li>
    % endif

    % if submodules:
    <li><h3><a class='nav-header' href="#header-submodules">Sub-modules</a></h3>
      <ul>
      % for m in submodules:
        <li><code>${link(m, sidebar=True)}</code></li>
      % endfor
      </ul>
    </li>
    % endif
    ...
    % if variables:
    <li><h3><a class='nav-header' href="#header-variables">Global variables</a></h3>
    ...
    <li><h3><a class='nav-header' href="#header-classes">Classes</a></h3>
      ...
      % for c in classes:
        <li>
        <h4><code>${link(c, sidebar=True)}</code></h4>
        <%
